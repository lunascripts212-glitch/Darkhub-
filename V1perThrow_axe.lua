...
-- Hook the RemoteEvent's FireServer via hookmetamethod to detect the axe being used.
-- OnClientEvent fires server→client; we need to intercept client→server (FireServer).
local function axeStartDetection()
    if axeHookConn then return end  -- already hooked

    -- hookmetamethod wraps __namecall so we see every :FireServer / :InvokeServer call
    local originalNC
    originalNC = hookmetamethod(game, "__namecall", function(self, ...)
        local method = getnamecallmethod()
        if method == "FireServer" and self == RemoteEvent then
            local args = { ... }
            -- args[1] = action string, args[2] = data table
            if args[1] == "UseActorAbility"
                and type(args[2]) == "table"
                and axeMatchesBuf(args[2][1])
                and axeLockEnabled then
    ...
    -- Store a reference so we can "unhook" by disabling the flag
    -- (hookmetamethod can't be fully reversed on most executors, so we gate with axeLockEnabled)
    axeHookConn = true
    print("[V1perThrow] Axe detection hooked (FireServer intercept).")
end

local function axeStopDetection()
    axeStopLock()
    -- Can't fully unhook hookmetamethod, but axeLockEnabled = false gates the lock trigger
    axeHookConn = nil
    print("[V1perThrow] Axe detection disabled.")
end
--
...
AxeTab:Paragraph({
    Title   = "How it works",
    Content = "Intercepts your FireServer call for the axe ability and locks your character facing the nearest killer for 1.7s (the animation duration). Camera never moves.",
})
